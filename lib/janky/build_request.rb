module Janky
  class BuildRequest
    def self.handle(repo_uri, branch_name, commit, compare, room)
      repos = Repository.find_all_by_uri(repo_uri)
      repos.each do |repo|
        begin
          new(repo, branch_name, commit, compare, room).handle
    ...
    def initialize(repo, branch_name, commit, compare, room)
      @repo        = repo
    ...
    def handle
      ...
      build = branch.build_for(commit, @room, @compare)
